pipeline {
    ...
    stages {
        ...
        stage('Publish to Git') {
            steps {
                withCredentials([usernamePassword(credentialsId: 'github-pat-cred',
                            passwordVariable: 'GIT_TOKEN', usernameVariable: 'GIT_USER')]) {
                    sh '''
